...
class RestfulExtension extends CompilerExtension
{
	...
	public function getConfigSchema(): Schema
	{
		return Expect::structure([
			'convention' => Expect::anyOf([
				self::CONVENTION_CAMEL_CASE,
				self::CONVENTION_PASCAL_CASE,
				self::CONVENTION_SNAKE_CASE,
				null,
			])->default(null),
			'timeFormat' => Expect::string('c'),
			'cacheDir' => Expect::string()->required(),
			'jsonpKey' => Expect::string('jsonp'),
			'mappers' => Expect::arrayOf(
				Expect::structure([
					'contentType' => Expect::string()->required(),
					'class' => Expect::string()->required(),
				])->castTo('array'),
			),
			'prettyPrint' => Expect::bool(true),
			'prettyPrintKey' => Expect::string('pretty'),
			'routes' => Expect::structure([
				'generateAtStart' => Expect::bool(false),
				'presentersRoot' => Expect::string()->required(),
				'autoGenerated' => Expect::bool(true),
				'autoRebuild' => Expect::bool(true),
				'module' => Expect::string(''),
				'prefix' => Expect::string(''),
				'panel' => Expect::bool(true),
			])->castTo('array'),
			'security' => Expect::structure([
				'privateKey' => Expect::anyOf(Expect::string(), Expect::null())->default(null),
				'requestTimeKey' => Expect::string('timestamp'),
				'requestTimeout' => Expect::int(300),
			])->castTo('array'),
		])->castTo('array');
	}
